describe('Counter App (Android)', () => {
  beforeEach(async () => {
    console.log('🤖 Starting Android test setup...');

    // Wait for app to load
    await driver.pause(2000);

    // Reset counter to 0 before each test (Android selectors)
    try {
      const resetButton = await $('//*[@content-desc="Reset"]');
  ...
  it('should display initial counter value of 0', async () => {
    ...
    // Use resource-id for counter value on Android
    const counterValue = await $('//*[@resource-id="counter-value"]');
  ...
  it('should increment counter when + button is pressed', async () => {
    ...
    const incrementButton = await $('//*[@content-desc="+"]');
    const counterValue = await $('//*[@resource-id="counter-value"]');
  ...
  it('should decrement counter when - button is pressed', async () => {
    ...
    const decrementButton = await $('//*[@content-desc="-"]');
    const counterValue = await $('//*[@resource-id="counter-value"]');
    const incrementButton = await $('//*[@content-desc="+"]');
